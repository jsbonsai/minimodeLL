...
- **MCP servers** (ADR 0012): Settings → MCP Servers edits user `mcpServers` only through `MCPServerStore` (refuses under forced policy, validates before atomic write, deletes only `mcp.<id>.`-owned Keychain items). Custom headers go through `MCPHeaderPolicy` (reserved names, injection-safe values; secrets by `secretAccount` only) and the SDK `requestModifier` in `MCPTransportFactory`.
