...
- `Tokens.swift`: `DesignPalette` (brand colors per scheme, plain values so offline rendering is deterministic; `onAccent` and the text-safe `runningInk`/`warningInk`/`blockedInk` alongside the dot colors), `Space`, `Radius`, `Typography` (Geist ramp), the `palette`/`opaqueMaterials` environment keys and `designRoot()`, which every design surface applies at its root (scheme → palette, base font, tint, Reduce Transparency → opaque materials).
...
- `CommandBarState.swift`: `CommandBarPhase`, `Destination` (built from the core's `InferenceDestination`: this Mac, LAN TLS/unencrypted, cloud), `CommandBarInputs` (plain facts) → `CommandBarModel.resolve` (pure; precedence locked › approval › running › error › result › typing › idle), `ProposalSummary`, `CommandBarActions`, `BarAction` list per phase. No policy or approval logic.
...
- `CommandBarController.swift`: owns the panel, hosting view (`preferredContentSize` sizing), hotkey registration (re-registers on `UserDefaults` change), `apply(_:)` (synchronous save + register for Settings), the local key monitor (`handle(_:)`, the keyboard map: key codes for special keys, `charactersIgnoringModifiers` for letters), show/hide with fade and a hide generation counter, approval surfacing via `withObservationTracking` without taking the keyboard (`ShowReason.approval`), the 0.6 s approval arming window (`armApproval`, one `decide` path, key repeats ignored, VoiceOver announcement), and `CommandBarHost` + `AppState.commandBarInputs`. Every action calls `AppState` (`submit`, `cancel`, `decide`, `stopRuntime`, `selectedModel`).
- `CommandBarSettings.swift`: Settings → Command Bar tab (shortcut text, validation notices, registration status, "Show command bar").
- `MinimodeLLApp.swift` creates the controller in `init`, puts `MenuBarExtra` first so no window opens at launch, adds `--render-design-previews`, `--open-command-bar` (debug: shows the bar 0.6 s after launch) and `--open-workspace` (debug: takes the ⌘O path), attaches `WorkspaceOpener` to the always-present `MenuBarExtra` label so ⌘O works on a fresh launch, and passes the controller to `MenuContent` and `SettingsView`.
